        ...
        if direction == 'haut':
            if self.y_box > 0:
                if self.lvl.structure[self.y_box-1][self.x_box] != 'm' and \
                    ...
                    self.y_box -= 1
                    self.y_pixel = self.y_box * constantes.SIZE_SPRITE
        # moving to down
        if direction == 'bas':
            if self.y_box < (constantes.NUMBER_SPRITE_FACE - 1):
                if self.lvl.structure[self.y_box+1][self.x_box] != 'm' and \
                        self.lvl.structure[self.y_box+1][self.x_box] != 'x':
                    self.y_box += 1
                    self.y_pixel = self.y_box * constantes.SIZE_SPRITE
